...
from storage import load_user_disk, save_user_disk, load_users_disk, save_users_disk,_user_file,_users_file
...
T = {
    "English": {
        ...
        "upload_cap": "Upload your invoice — OCR reads and auto-detects whether it is a Purchase or Sales invoice.",
        "smart_valid": "ℹ️ Invoices are validated before being added. Invalid GSTINs and illegal tax charges are automatically rejected.",
        "drop_invoice": "Drop invoice here", "extract_btn": "🔍 Extract Data",
        "reading_invoice": "Reading invoice...", "reading": "Reading invoice...",
        "extraction_complete": "✓ Extraction complete!",
        "extraction_failed": "Extraction failed:",
        "extracted_fields": "**🔎 Extracted Fields**",
        "detection": "Detection:", "ocr_confidence": "OCR Confidence", "ocr_conf": "OCR Confidence",
        "illegal_tax": "🚨 ILLEGAL TAX ALERT — No GSTIN but GST charged. Cannot claim ITC. Demand corrected invoice.",
        "no_gstin_warn": "⚠️ No GSTIN",
        "vendor_party": "Vendor / Party", "invoice_type": "Invoice Type",
        "inv_type": "Invoice Type",
        "date_col": "Date", "date": "Date", "gstin_col": "GSTIN", "gstin": "GSTIN",
        "no_gstin": "⚠️ No GSTIN",
        "category_col": "Category", "subtotal_col": "Subtotal", "subtotal_lbl": "Subtotal",
        "cgst_col": "CGST", "cgst": "CGST", "sgst_col": "SGST", "sgst": "SGST",
        "igst_col": "IGST", "igst": "IGST",
        "total_amount": "Total Amount", "total_amt": "Total Amount",
        "field_col": "Field", "field": "Field", "value_col": "Value", "value": "Value",
        "review_confirm": "**✏️ Review & Confirm**",
        "vendor_name": "Vendor / Party Name", "vendor": "Vendor / Party Name",
        "txn_date": "Date",
        "calculated_total": "Calculated Total", "calc_total": "Calculated Total",
        "confirm_btn": "✅ Confirm & Add to Register", "confirm_add": "✅ Confirm & Add to Register",
        ...
        "inv_types": ["Purchase Invoice", "Sales Invoice", "Credit Note", "Debit Note", "Expense Receipt"],
        "to_sales_reg": "Sales Register", "to_purchase_reg": "Purchase Register",
        "sales_register": "Sales Register", "purchase_register": "Purchase Register",
...
| 4️⃣ | **Position** | Your GSTIN in top half → Sales, else Purchase |""",
        ...
        "rejected_title": "🚫 Invoice Rejected",
        "not_added": "This invoice has NOT been added to your register.",
        "what_next": "**What to do next:** Contact the supplier and request a corrected invoice with a valid GSTIN.",
        "upload_rejected_box": "Invoice was rejected due to compliance issues. Upload a valid invoice.",
        "reject_no_gstin": (
    ...
    "ಕನ್ನಡ": {
        ...
        "upload_caption": "ನಿಮ್ಮ ಬಿಲ್ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.",
        "upload_cap": "ನಿಮ್ಮ ಬಿಲ್ ಅಪ್‌ಲೋಡ್ ಮಾಡಿ.",
        "smart_valid": "ℹ️ ಬಿಲ್‌ಗಳನ್ನು ಸೇರಿಸುವ ಮೊದಲು ಪರಿಶೀಲಿಸಲಾಗುತ್ತದೆ.",
        "drop_invoice": "ಬಿಲ್ ಇಲ್ಲಿ ಹಾಕಿ", "extract_btn": "🔍 ಮಾಹಿತಿ ತೆಗೆಯಿರಿ",
        "reading_invoice": "ಬಿಲ್ ಓದಲಾಗುತ್ತಿದೆ...", "reading": "ಬಿಲ್ ಓದಲಾಗುತ್ತಿದೆ...",
        ...
        "ocr_conf": "ಓಸಿಆರ್ ವಿಶ್ವಾಸಾರ್ಹತೆ",
        "illegal_tax": "🚨 ಕಾನೂನು ಬಾಹಿರ ತೆರಿಗೆ.",
        "no_gstin_warn": "⚠️ ಜಿಎಸ್‌ಟಿಐಎನ್ ಇಲ್ಲ",
        "vendor_party": "ಮಾರಾಟಗಾರ / ಪಾರ್ಟಿ", "invoice_type": "ಬಿಲ್ ವಿಧ",
        "inv_type": "ಬಿಲ್ ವಿಧ",
        "date_col": "ದಿನಾಂಕ", "date": "ದಿನಾಂಕ",
        "gstin_col": "ಜಿಎಸ್‌ಟಿಐಎನ್", "gstin": "ಜಿಎಸ್‌ಟಿಐಎನ್",
        "no_gstin": "⚠️ ಜಿಎಸ್‌ಟಿಐಎನ್ ಇಲ್ಲ", "category_col": "ವರ್ಗ",
        "subtotal_col": "ಒಳಮೊತ್ತ", "subtotal_lbl": "ಒಳಮೊತ್ತ",
        "cgst_col": "ಸಿಜಿಎಸ್‌ಟಿ", "cgst": "ಸಿಜಿಎಸ್‌ಟಿ",
        "sgst_col": "ಎಸ್‌ಜಿಎಸ್‌ಟಿ", "sgst": "ಎಸ್‌ಜಿಎಸ್‌ಟಿ",
        "igst_col": "ಐಜಿಎಸ್‌ಟಿ", "igst": "ಐಜಿಎಸ್‌ಟಿ",
        "total_amount": "ಒಟ್ಟು ಮೊತ್ತ", "total_amt": "ಒಟ್ಟು ಮೊತ್ತ",
        "field_col": "ಅಂಶ", "field": "ಅಂಶ", "value_col": "ಮೌಲ್ಯ", "value": "ಮೌಲ್ಯ",
        "review_confirm": "**✏️ ಪರಿಶೀಲಿಸಿ & ದೃಢಪಡಿಸಿ**",
        "vendor_name": "ಮಾರಾಟಗಾರ / ಪಾರ್ಟಿ ಹೆಸರು", "vendor": "ಮಾರಾಟಗಾರ / ಪಾರ್ಟಿ ಹೆಸರು",
        "txn_date": "ದಿನಾಂಕ",
        "calculated_total": "ಲೆಕ್ಕ ಮಾಡಿದ ಒಟ್ಟು", "calc_total": "ಲೆಕ್ಕ ಮಾಡಿದ ಒಟ್ಟು",
        "confirm_btn": "✅ ದೃಢಪಡಿಸಿ & ನೋಂದಣಿಗೆ ಸೇರಿಸಿ",
        "confirm_add": "✅ ದೃಢಪಡಿಸಿ & ನೋಂದಣಿಗೆ ಸೇರಿಸಿ",
        ...
        "inv_types": ["ಖರೀದಿ ಬಿಲ್", "ಮಾರಾಟ ಬಿಲ್", "ಕ್ರೆಡಿಟ್ ನೋಟ್", "ಡೆಬಿಟ್ ನೋಟ್", "ಖರ್ಚು ರಶೀದಿ"],
        "to_sales_reg": "ಮಾರಾಟ ನೋಂದಣಿ", "to_purchase_reg": "ಖರೀದಿ ನೋಂದಣಿ",
        "sales_register": "ಮಾರಾಟ ನೋಂದಣಿ", "purchase_register": "ಖರೀದಿ ನೋಂದಣಿ",
        ...
        "rejected_title": "🚫 ಬಿಲ್ ತಿರಸ್ಕರಿಸಲಾಗಿದೆ",
        "not_added": "ಈ ಬಿಲ್ ಅನ್ನು ನಿಮ್ಮ ನೋಂದಣಿಗೆ ಸೇರಿಸಲಾಗಿಲ್ಲ.",
        "what_next": "**ಮುಂದೇನು:** ಪೂರೈಕೆದಾರರನ್ನು ಸಂಪರ್ಕಿಸಿ ಮತ್ತು ಮಾನ್ಯ GSTIN ಸಹಿತ ಸರಿಪಡಿಸಿದ ಬಿಲ್ ಕೇಳಿ.",
        "upload_rejected_box": "ಅನುಸರಣೆ ಸಮಸ್ಯೆಗಳಿಂದ ಬಿಲ್ ತಿರಸ್ಕರಿಸಲಾಗಿದೆ.",
...
def add_to_register(data, doc_type, category="Goods"):
...
def build_gstr2a_2b(pdf,g2a):
    ...
    for _,row in pr.iterrows():
        ...
        if matched.empty: status=t("not_in_2a")
            ...
            a_s=matched["SGST"].astype(float).sum()
            a_i=matched["IGST"].astype(float).sum()
            status=(t("matched") if abs(float(row["CGST"])-a_c)<1. or
                                    abs(float(row["SGST"])-a_s)<1. or
                                    abs(float(row["IGST"])-a_i)<1. else t("mismatch"))
        rows.append({"ID":row["ID"],"Vendor":str(row.get("Vendor","")),"GSTIN":gstin,
                     "Your CGST":row["CGST"],"Your SGST":row["SGST"],"Your IGST":row["IGST"],
                     "2A CGST":matched["CGST"].astype(float).sum() if not matched.empty else 0.,
                     "2A SGST":matched["SGST"].astype(float).sum() if not matched.empty else 0.,
...
def validate_invoice_before_upload(extracted: dict) -> tuple:
...
def parse_invoice_text(text):
    ...
    if bi is not None:
        sl = lines[:bi]          # seller / from section
        bl = lines[bi+1:bi+10]  # bill-to / buyer section
    else:
        mid = len(lines) // 2
        sl = lines[:mid]
        bl = lines[mid:]

    iam_s = has_me(sl)  # our identity appears in seller block → we are seller → Sales
    iam_b = has_me(bl)  # our identity appears in buyer block  → we are buyer  → Purchase

    # ── FIXED invoice type detection — GSTIN position takes priority ────────
    # Priority 1: GSTIN position in document (most reliable)
    if iam_s and not iam_b:
        dt = "Sales Invoice"
    elif iam_b and not iam_s:
        dt = "Purchase Invoice"
    # Priority 2: Explicit keyword on the invoice
    elif re.search(r'sales\s*invoice', full, re.IGNORECASE):
        dt = "Sales Invoice"
        ...
        dt = "Debit Note"
    # Priority 3: Our GSTIN anywhere in top half → we are the seller
    else:
    ...
    # ── Skip patterns for vendor extraction ────────────────────────────────
    sp = [
        r'\d{2}[A-Za-z]{5}\d{4}',
        ...
        r'description|hsn|sac|qty|rate|amount|sl\.?\s*no|item|particulars|unit|price',
    ]
    ...
    # For Sales invoice: vendor = the buyer (bill-to block)
    # For Purchase invoice: vendor = the seller (from block)
    ...
    def extract_tax(keyword):
        patterns = [
            rf'{keyword}\s*@\s*\d+\.?\d*\s*%\s*(?:Rs\.?|INR|₹)?\s*([0-9,]{{2,}}\.?\d*)',
            rf'{keyword}\s*[:\-]\s*(?:Rs\.?|INR|₹)?\s*([0-9,]{{2,}}\.?\d*)',
            rf'{keyword}\s+(?:Rs\.?|INR|₹)?\s*([0-9,]{{2,}}\.?\d*)',
        ]
        result = fa(patterns, full)
        if result > 0:
            return result
        multiline_pat = rf'{keyword}[^\n]{{0,30}}\n[^\n]{{0,20}}%[^\n]{{0,20}}\n\s*(?:Rs\.?|INR|₹)?\s*([0-9,]{{2,}}\.?\d*)'
        m = re.search(multiline_pat, text, re.IGNORECASE)
        if m:
            try:
                return float(m.group(1).replace(",", "").replace(" ", ""))
            except:
    ...
    # ── Back-calculate tax from total − subtotal ────────────────────────────
    if tax == 0. and total > 0 and sub > 0:
        ...
        if derived_tax > 0:
            if hc and hs and not hi:
                ...
                cgst = derived_tax
            elif hs and not hc and not hi:
                sgst = derived_tax
            tax = cgst + sgst + igst

    # ── Derive subtotal from total − tax ────────────────────────────────────
...
def show_login():
    ...
    [data-testid="stAppViewContainer"]::before {
        ...
        background: rgba(8, 10, 15, 0.82); backdrop-filter: blur(2px); z-index: 0;
    }
    ...
    with c1:
        ...
        if st.session_state.get("upload_rejected"):
            ...
            <div class="reject-banner">
                <div class="reject-title">{t("rejected_title")}</div>
                <div class="reject-body">{reason.replace(chr(10),'<br/>')}</div>
            </div>""", unsafe_allow_html=True)
    ...
    with c2:
        ...
        if st.session_state.extracted and not st.session_state.get("upload_rejected"):
            ...
            with st.form("confirm_form"):
                ...
                # Map detected English doc_type to the translated inv_types list index
                _EN_TYPES    = ["Purchase Invoice","Sales Invoice","Credit Note","Debit Note","Expense Receipt"]
                _inv_types   = t("inv_types")
                _detected_en = ed.get("doc_type", "Purchase Invoice")
                _detected_idx = _EN_TYPES.index(_detected_en) if _detected_en in _EN_TYPES else 0
                doc_type = st.selectbox(t("inv_type"), _inv_types, index=_detected_idx)
                txn_date = st.text_input(t("date"),      value=ed.get("date",""))
                gstin    = st.text_input(t("gstin"),     value=ed.get("gstin",""))
                sub      = st.number_input(t("subtotal_lbl"), value=float(ed.get("subtotal",0)), min_value=0., step=0.01)
                cgst = sgst = igst = 0.
                ...
                if st.form_submit_button(t("confirm_add"), use_container_width=True):
                    ...
                    doc_type_en = EN_TYPES[inv_types_list.index(doc_type)] if doc_type in inv_types_list else doc_type
                    data = {
...
# ══════════════════════════════════════════════
    ...
    with c1:
        ...
        with st.form("manual_form"):
            ...
            with cc1:
                cat=st.selectbox(t("category_col"),t("categories"))
            ...
            if st.form_submit_button(t("add_entry_btn"),use_container_width=True):
                if not vendor: st.error(t("vendor_required"))
                    ...
                    txn_id,reg=add_to_register(data,dt_en,category=cat)
...
# ══════════════════════════════════════════════
    ...
    if sdf.empty: st.warning(t("no_sales_gstr1"))
        ...
        st.markdown("---")
        tb1,tb2,tb3,tb4=st.tabs([t("b2b_tab"),t("b2c_tab"),t("hsn_tab"),t("full_table_tab")])
        with tb1:
        ...
        with tb2:
            ...
            if b2c.empty: st.info(t("no_b2c"))
                ...
                st.dataframe(d,use_container_width=True,hide_index=True)
        with tb3:
            st.caption(t("hsn_caption"))
            if "Category" in sdf.columns:
                hsn=sdf.groupby("Category").agg(
                    ID=("ID","count"),Subtotal=("Subtotal","sum"),
                    CGST=("CGST","sum"),SGST=("SGST","sum"),
                    IGST=("IGST","sum"),Total=("Total","sum")
                ).reset_index()
                hsn.columns=["Category",t("invoices_col"),t("subtotal_col"),"CGST","SGST","IGST",t("total_col")]
                for c in [t("subtotal_col"),"CGST","SGST","IGST",t("total_col")]: hsn[c]=hsn[c].apply(fmt_inr)
                st.dataframe(hsn,use_container_width=True,hide_index=True)
            else: st.info("No category data available.")
        with tb4:
...
# ══════════════════════════════════════════════
    ...
    with tb3:
        ...
        if pdf_.empty: st.info(t("no_purchase_recon"))
            ...
            d=recon_df.copy()
            for c in ["Your CGST","Your SGST","Your IGST","2A CGST","2A SGST","2A IGST"]: d[c]=d[c].apply(fmt_inr)
            st.dataframe(d,use_container_width=True,hide_index=True)
